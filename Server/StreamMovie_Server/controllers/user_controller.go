...
// isProductionEnv controls how the auth cookies are issued. Locally (the default) we use
// Secure=false + SameSite=Lax so Postman/browsers actually store the cookie when the API is
// served over plain http://localhost — a Secure cookie is silently dropped over http. Set
// APP_ENV=production once the API is served over HTTPS to get Secure + SameSite=None back.
