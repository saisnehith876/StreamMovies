...
func GetReviewRanking(admin_review string, client *mongo.Client, c *gin.Context) (string, int, error) {
	...
	// Groq exposes an OpenAI-compatible endpoint, so we reuse the OpenAI client and just
	// point it at Groq's base URL + model. GROQ_API_KEY is preferred; OPENAI_API_KEY is
	// kept as a fallback in case the .env file hasn't been renamed yet.
	GroqApiKey := os.Getenv("GROQ_API_KEY")
	if GroqApiKey == "" {
		GroqApiKey = os.Getenv("OPENAI_API_KEY")
	}

	if GroqApiKey == "" {
		return "", 0, errors.New("could not read GROQ_API_KEY")
	}
...
var usersCollection *mongo.Collection = database.OpenCollection("Users", database.Client)
...
//GetReviewRanking error: API returned unexpected status code: 429:
// You exceeded your current quota, please check your plan and billing details. For more information on this error,

//So use groq api instead of openai api, as groq api is free and openai api is paid.
...
func GetUsersFavouriteGenres(userId string) ([]string, error) {
	...
	fmt.Println("DEBUG result:", result) // add this
